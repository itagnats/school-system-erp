...
import { catalogueItemCreateSchema } from "@/lib/api/contracts";
import { jsonError, notFound } from "@/server/http";
import { parseBody, readJson } from "@/server/validation";
import { createCatalogueItem } from "@/server/services";
 ...
 * catalogue by group, so the group is the unit the cache holds, and returning
...
export async function POST(
  ...
  const parsed = parseBody(catalogueItemCreateSchema, await readJson(request));
  if (!parsed.ok) {
    return jsonError(422, "Some fields need attention", parsed.fieldErrors);
  }

  const updated = createCatalogueItem(groupId, parsed.data);
