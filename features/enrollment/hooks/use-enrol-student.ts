...
import type { EnrolmentResult, EnrollmentListItem, PaginatedResult } from "@/types";
 ...
 * back and visibly undo the enrolment a second after it was made. That reads
 * as a bug rather than as a documented scope boundary.
 *
 * One enrolment produces several rows, which is the part that differs from
...
export function useEnrolStudent() {
  ...
  return useMutation({
    ...
    onSuccess: (result: EnrolmentResult) => {
 ...
 *
 * Disabled until a programme is known, because "every student" is not a useful
 * list and a request for it would only be thrown away. `semester` switches the
 * question from "on this programme" to "on this programme last term", which is
