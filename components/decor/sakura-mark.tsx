 ...
 * Two variants:
 *
 *   seal   flat, one colour, a notch at each petal tip. Reads at 16px, which
 ...
 *
 * Both keep the single-colour contract. `bloom` gets its depth from alpha
...
/** The `seal` petal: a rounded lobe rising from the centre, notched at the tip. */
const SEAL_PETAL =
...
/** The `bloom` petal: a broad leaf sweeping out from the centre. */
    ...
    return (
      <svg viewBox="0 0 100 100" className={cn("size-4", className)} fill="none" {...a11y}>
        ...
        {/* Stamen. Filaments are drawn first so the disc covers where they
            leave the centre, which is what makes them read as radiating. */}
